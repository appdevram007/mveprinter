...
export default function PrinterApp() {
  ...
  const fetchPrintJobs = async () => {
    try {
     ...
     let resposn= await orderApi.getAllOrders()
     let SAMPLE_ORDERS=resposn.order
      ...
      const transformedJobs = SAMPLE_ORDERS.map(order => ({
        ...
        printTimestamp: order.createdAt, // Using createdAt as print timestamp
 ...
 // ===== CUSTOMER INFO =====
  ...
  const handlePrintJob = async (job) => {
    ...
    // Add job to printJobs with current timestamp
    const jobWithTimestamp = {
      ...job,
      printTimestamp: new Date().toISOString(),
      status: 'printed'
    };
    setPrintJobs(prev => [jobWithTimestamp, ...prev]);
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      return () => {
        s?.disconnect();
        s?.off("print_job", handlePrintJob);
      };
    }
  }, [isAuthenticated]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <Text style={styles.headerTitle}>Printer Dashboard</Text>
              ...
              {selectedReceipt && (
                <>
                  <View style={styles.receiptHeader}>
                    <Text style={styles.receiptShopName}>MY SHOP</Text>
